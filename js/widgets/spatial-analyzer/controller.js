...
import { SpatialAnalyzerWidget } from '../spatial-analyzer.js';
...
export async function openSpatialAnalyzer(ctx) {
    ...
    if (!_legacyWidget) {
        _legacyWidget = new SpatialAnalyzerWidget();
    }
    _legacyWidget.getLayers = ctx.getLayers;
    _legacyWidget.getLayerById = ctx.getLayerById;
    _legacyWidget.mapService = ctx.mapService;
    _legacyWidget.addLayer = ctx.addLayer;
    _legacyWidget.createSpatialDataset = ctx.createSpatialDataset;
